...
/** The receipt itself — rendered twice by <Receipt>: once in place for the screen, once portaled to <body> for print. */
function ReceiptCard({ payment, patientName, serviceName, branchName }: ReceiptProps) {
  ...
  return (
    <Card className="receipt-card relative mx-auto w-full max-w-md overflow-hidden">
      {/* Watermark — the same logo, faint and oversized, sitting behind the content only. */}
      <div
        aria-hidden
        className="pointer-events-none absolute inset-0 z-0 flex items-center justify-center"
      >
        <Image src="/logo.png" alt="" width={260} height={260} className="opacity-[0.06] grayscale" />
      </div>
      ...
      <div className="relative z-10">
        ...
        <dl className="mt-4 grid grid-cols-2 gap-x-4 gap-y-3">
          ...
          <MetaField label="Payment Method" value={payment.method.replace("_", " ")} />
...
export function Receipt(props: ReceiptProps) {
  // The receipt can be shown inside a Modal or a Drawer (position: fixed + an active transform), both of
  // which become the CSS containing block for any absolutely-positioned descendant. That traps a
  // print-only "fill the page" element inside the drawer's own narrow, right-pinned box instead of the
  // true page — which is exactly the bug this sidesteps: portal a dedicated print copy directly onto
  // <body>, outside every modal/drawer's containing block, so #print-area always measures against the
  // real page regardless of where the on-screen receipt happens to be shown.
  //
  // No mount-state guard needed for the SSR/document check below: Receipt only ever
  // appears after a client-side interaction (a completed payment step inside a modal or
  // wizard), never as part of the initial server-rendered tree, so `document` is always
  // defined by the time this renders — there's nothing to reconcile against on hydration.
  return (
    <div className="flex flex-col gap-4">
      ...
      <Button variant="secondary" onClick={() => window.print()} className="mx-auto print:hidden">
